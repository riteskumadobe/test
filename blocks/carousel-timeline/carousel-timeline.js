...
function createSlide(row, slideIndex, carouselTimelineId) {
  const slide = document.createElement('li');
  slide.dataset.slideIndex = slideIndex;
  slide.setAttribute('id', `carousel-timeline-${carouselTimelineId}-slide-${slideIndex}`);
...
let carouselTimelineId = 0;
export default async function decorate(block) {
  carouselTimelineId += 1;
  block.setAttribute('id', `carousel-timeline-${carouselTimelineId}`);
  ...
  rows.forEach((row, idx) => {
    const slide = createSlide(row, idx, carouselTimelineId);
